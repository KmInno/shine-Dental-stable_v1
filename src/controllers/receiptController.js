...
async function updateReceipt(req, res, next) {
    try {
        ...
        if (!patient_name || !patient_name.trim()) {
            req.flash("error", "Patient name is required");
            return res.redirect(`/receipts/edit/${receipt_id}`);
        }

        if (!patient_phone || !patient_phone.trim()) {
            req.flash("error", "Patient phone is required");
            return res.redirect(`/receipts/edit/${receipt_id}`);
        }
        ...
        const updated = await ReceiptItem.updateReceipt(
            ...
            patient_next_visit && patient_next_visit.trim() ? patient_next_visit : null,
            room_number && room_number.trim() ? room_number : null,
            service || "",
            ...
            balance !== undefined && balance !== null && balance !== "" ? parseFloat(balance) : (parseFloat(total) || 0) - (parseFloat(amount_paid) || 0)
        );
...
async function editReceiptForm(req, res, next) {
    try {
        const receipt_id = req.params.receipt_id;
        const receipt = await ReceiptItem.getReceiptDetails(receipt_id);

        if (!receipt) {
            logger.warn(`Receipt with ID ${receipt_id} not found for edit`);
            return res.status(404).render("error", {
                title: "Error",
                message: "Receipt not found",
                user: req.user
        ...
        logger.error(`Error fetching receipt for edit: ${error.message}`, error);
        next(error);
    }
}
...
async function viewPatientRecords(req, res) {
    try {
        ...
        const existingPatients = await patientService.findPatientsByName(patient_name);
        ...
        return res.render("patientRecords", {
            ...
            history: patientReceipts.map(receipt => ({
                ...receipt,
                visit_date: receipt.visit_date || receipt.created_at
            })),
            user: req.user
        });
    } catch (error) {
...
async function checkDuplicateName(req, res) {
    try {
        ...
        const existingPatients = await patientService.findPatientsByName(patient_name.trim());
...
module.exports = { addReceipt, getAllReceipts, receiptDetails, deleteReciept, updateReceipt, editReceiptForm, viewPatientRecords, viewSpecificPatientRecords, searchOldPatients, checkDuplicateName, validatePatientId };

async function getPatientRecordsJson(req, res) {
    try {
        ...
        const patientDetails = await patientService.getPatientDetails(patient_phone);
        const patientReceipts = await patientService.getReceiptsByPatient(patient_phone);

        if (patientReceipts && patientReceipts.forEach) {
            patientReceipts.forEach(receipt => {
        ...
        return res.json({ success: true, patientDetails, patientReceipts });
    } catch (error) {
        logger.error(`Error in getPatientRecordsJson: ${error.message}`, error);
        return res.status(500).json({ success: false, message: 'Error fetching patient records' });
    }
}

// add JSON endpoint to exports
module.exports.getPatientRecordsJson = getPatientRecordsJson;
...
async function viewSpecificPatientRecords(req, res) {
    try {
        ...
        return res.render("patientRecords", {
            ...
            patient: {
                ...patientDetails,
                next_visit: patientDetails.next_visit || null
            },
            history: patientReceipts.map(receipt => ({
                ...receipt,
                visit_date: receipt.visit_date || receipt.created_at
